...
<div class="container-fluid">
   <ul class="nav nav-tabs nav-tabs__service"><br>
      ...
      <li class="pull-right">
        ...
        <button class="btn btn-primary btn-lg" data-toggle="modal" data-target="#modalDiv" data-action="import_services">Import Services In to Categories</button></li>
   
<button class="btn btn-primary btn-lg" data-toggle="modal" data-target="#modalDiv" data-action="import_service">Import Services With Categories</button></li>
   </ul>
   <ul></ul>
...
<th class="service-block__minorder">Service Type    </th> 
<th class="service-block__minorder">Refill</th>
<th class="service-block__minorder">Cancel</th>
<th class="service-block__minorder">Provider</th>
 ...
 <th class="service-block__visibility">Action</th>
                                    ...
                                    <?php for($i=0;$i<count($services);$i++): $api_detail = json_decode($services[$i]["api_detail"],true); ?>
                                       <tr id="serviceshowcategory-<?=$c?>" class="ui-state-default <?php if( $services[$i]["service_type"] == 1 ): echo "grey"; endif; ?>"  data-category="category-<?=$c?>" data-id="service-<?php echo $services[$i]["service_id"] ?>" data-service="<?php echo $services[$i]["service_name"] ?>">
                                         <?php if( !empty($services[0]["service_id"])  ):?>
                                            ...
                                            <td class="service-block__rate"><?php echo servicePackageType($services[$i]["service_package"]); ?><?php if( $services[$i]["show_refill"] == "true" ): echo '<div  class="tooltip5"><i  class="fas fa-repeat"></i></span><span class="tooltiptext5" >Refill button Enabled</span></div>'; endif; ?>
...
<?php if($services[$i]["time"] != "Not enough data" ): ?><div  class="tooltip5"><i  class="fas fa-rocket"></i></span><span class="tooltiptext5" >Average Time : <?php echo $services[$i]["time"]; ?></span></div> <?php endif; ?>
</td>
                                            										    <?php if( $services[$i]["show_refill"] == "true" ): $type = "refill-deactive"; else: $type = "refill-active"; endif; ?>
										   <td  class="service-block__rate"> <a href="<?php echo site_url("admin/services/".$type."/".$services[$i]["service_id"]) ?>"> <?php if( $services[$i]["show_refill"] == "false" ): echo "Off"; else: echo "On"; endif; ?></a></td>
										   
					<?php if( $services[$i]["cancelbutton"] == 2 ): $type = "cancelbutton-active"; else: $type = "cancelbutton-deactive"; endif; ?>
									   <td  class="service-block__rate"> <a href="<?php echo site_url("admin/services/".$type."/".$services[$i]["service_id"]) ?>"> <?php if( $services[$i]["cancelbutton"] == "2" ): echo "Off"; else: echo "On"; endif; ?></a></td>
										   
										   
                                            <td class="service-block__visibility"><?php if( $services[$i]["service_api"] != 0 ): echo $services[$i]["api_name"]; else: echo "Manual"; endif; ?></td>

                                            <td class="service-block__rate">
                                              <?php
                                                if( $api_detail["currency"] == "INR" ):
                                                  $api_price  = $api_detail["rate"]/$settings["dolar_charge"];
                                                elseif( $api_detail["currency"] == "USD" ):
