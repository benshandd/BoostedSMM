...
$there_change=0;
         ...
         $detail["min"]=$apiService["min"];$detail["max"]=$apiService["max"];$detail["rate"]=$apiService["rate"];$detail["currency"]=$balance->currency;$detail=json_encode($detail);
          $extras = json_decode($service["api_detail"],true);
            if( $apiService["rate"] != $extras["rate"] ):
              ...
              $insert->execute(array("service"=>$service["service_id"],"alert"=>"#".$service["service_id"]."Number of service price has been changed.","date"=>date("Y-m-d H:i:s"),"extra"=>json_encode($extra) ));
            ...
            if( $apiService["min"] != $extras["min"] ):
              ...
              $insert = $conn->prepare("INSERT INTO serviceapi_alert SET service_id=:service, serviceapi_alert=:alert, servicealert_date=:date, servicealert_extra=:extra ");
              $insert->execute(array("service"=>$service["service_id"],"alert"=>"#".$service["service_id"]."Number of service minimum amount changed.","date"=>date("Y-m-d H:i:s"),"extra"=>json_encode($extra) ));
              if( $insert ): $there_change = $there_change+1; endif;
            endif;
            if( $apiService["max"] != $extras["max"] ):
              $extra  = ["old"=>$extras["max"],"new"=>$apiService["max"] ];
              $insert = $conn->prepare("INSERT INTO serviceapi_alert SET service_id=:service, serviceapi_alert=:alert, servicealert_date=:date, servicealert_extra=:extra ");
              $insert->execute(array("service"=>$service["service_id"],"alert"=>"#".$service["service_id"]."Number of service maximum amount has been changed.","date"=>date("Y-m-d H:i:s"),"extra"=>json_encode($extra) ));
              if( $insert ): $there_change = $there_change+1; endif;
            endif;
              if( $service["api_servicetype"] == 1 && $there[$service["service_id"]] ):
                $extra  = ["old"=>"Inactive on Provider","new"=>"Active on Provider" ];
                ...
                $insert = $conn->prepare("INSERT INTO serviceapi_alert SET service_id=:service, serviceapi_alert=:alert, servicealert_date=:date, servicealert_extra=:extra ");
                $insert->execute(array("service"=>$service["service_id"],"alert"=>"#".$service["service_id"]."Re-activated by number service provider.","date"=>date("Y-m-d H:i:s"),"extra"=>json_encode($extra) ));
                if( $insert ): $there_change = $there_change+1; endif;
              else:
                $update = $conn->prepare("UPDATE services SET api_detail=:detail, api_servicetype=:type WHERE service_id=:service ");
                $update->execute(array("service"=>$service["service_id"],"detail"=>$detail,"type"=>2 ));
  ...
  foreach ($there as $service => $type):
    ...
    if( $type == 0 && $serviceDetail["api_servicetype"] == 2 ):
      $extra  = ["old"=>"Active on Provider","new"=>"Passive on Provider" ];
      ...
      $insert = $conn->prepare("INSERT INTO serviceapi_alert SET service_id=:service, serviceapi_alert=:alert, servicealert_date=:date, servicealert_extra=:extra ");
      $insert->execute(array("service"=>$service,"alert"=>"#".$service."removed by the number service provider.","date"=>date("Y-m-d H:i:s"),"extra"=>json_encode($extra) ));
  ...
  if( $settings["alert_serviceapialert"] == 2 && $there_change ):
    ...
    if( $sendsms ):
      SMSUser($settings["admin_telephone"],"You have services whose information is changed by the service provider.");
    endif;
    if( $sendmail ):
      sendMail(["subject"=>"New order available.","body"=>"You have services whose information is changed by the service provider.","mail"=>$settings["admin_mail"]]);
    endif;
  endif;
