...
if( $panel["panel_status"] == "frozen" ): include 'app/views/frozen.twig';exit(); endif;
  ...
  if( route(2)  ==  "counter" ):
    ...
    echo '<li'; if( !$active ): echo ' class="active"'; endif; echo '>
            <a href="/admin/apiorders/all">All Orders ('.$count.')</a>
          </li>';
...
if( $_GET["search_type"] == "username" && $_GET["search"] ):
      ...
      $count        = $conn->prepare("SELECT * FROM orders INNER JOIN clients ON clients.client_id = orders.client_id WHERE {$search} && orders.dripfeed='1' && orders.order_where='api' && orders.subscriptions_type='1' ");
      $count        -> execute(array());
      $count        = $count->rowCount();
      $search       = "WHERE {$search} && orders.dripfeed='1' && orders.order_where='api' && orders.subscriptions_type='1' ";
      $search_link  = "?search=".$search_word."&search_type=".$search_where;
    elseif( $_GET["search_type"] == "order_id" && $_GET["search"] ):
      $search_where = $_GET["search_type"];
      $search_word  = urldecode($_GET["search"]);
      $count        = $conn->prepare("SELECT * FROM orders INNER JOIN clients ON clients.client_id = orders.client_id WHERE orders.order_id LIKE '%".$search_word."%' && orders.dripfeed='1' && orders.order_where='api' && orders.subscriptions_type='1' ");
      $count        -> execute(array());
      $count        = $count->rowCount();
      $search       = "WHERE orders.order_id LIKE '%".$search_word."%'  && orders.dripfeed='1' && orders.order_where='api' && orders.subscriptions_type='1' ";
      $search_link  = "?search=".$search_word."&search_type=".$search_where;
    elseif( $_GET["search_type"] == "order_url" && $_GET["search"] ):
      $search_where = $_GET["search_type"];
      $search_word  = urldecode($_GET["search"]);
      $count        = $conn->prepare("SELECT * FROM orders INNER JOIN clients ON clients.client_id = orders.client_id WHERE orders.order_url LIKE '%".$search_word."%' &&orders.order_where='api' && orders.dripfeed='1' && orders.subscriptions_type='1' ");
      $count        -> execute(array());
      $count        = $count->rowCount();
      $search       = "WHERE orders.order_url LIKE '%".$search_word."%'  && orders.dripfeed='1' && orders.order_where='api' && orders.subscriptions_type='1' ";
      $search_link  = "?search=".$search_word."&search_type=".$search_where;
    elseif( $_GET["subscription"] ):
      ...
      $count        -> execute(array());
      $count        = $count->rowCount();
      $search       = "WHERE orders.order_where='api' && orders.dripfeed='1' && orders.subscriptions_type='1' ";
    ...
    elseif( $_GET["dripfeed"] ):
      ...
      $count        -> execute(array());
      $count        = $count->rowCount();
      $search       = "WHERE orders.dripfeed_id='$drip_id'  && orders.dripfeed='1'&& orders.order_where='api'  && orders.subscriptions_type='1' ";
    ...
    $failCount      = $conn->prepare("SELECT * FROM orders WHERE orders.dripfeed='1' && orders.order_where='api' && orders.subscriptions_type='1' && order_error!=:error ");
    $failCount     -> execute(array("error"=>"-"));
    $failCount      = $failCount->rowCount();
	
	
	//Cron bekleniyor
	$cronpendingcount      = $conn->prepare("SELECT * FROM orders WHERE orders.dripfeed='2' && orders.subscriptions_type='1' && dripfeed_status=:dripfeed_status");
    $cronpendingcount     -> execute(array("dripfeed_status"=>"active"));
    $cronpendingcount      = $cronpendingcount->rowCount();
	
	
	/// Yükleniyor
	$inprogresscount      = $conn->prepare("SELECT * FROM orders WHERE order_status=:order_status && order_where=:where");
    $inprogresscount     -> execute(array("order_status"=>"inprogress","where"=>"api"));
    $inprogresscount      = $inprogresscount->rowCount();
	
	//Tamamlandı
	$completedcount      = $conn->prepare("SELECT * FROM orders WHERE order_status=:order_status && order_where=:where");
    $completedcount     -> execute(array("order_status"=>"completed","where"=>"api"));
    $completedcount      = $completedcount->rowCount();
	
	//Kısmen Tamamlandı
	$partialcount      = $conn->prepare("SELECT * FROM orders WHERE order_status=:order_status && order_where=:where");
    $partialcount     -> execute(array("order_status"=>"partial","where"=>"api"));
    $partialcount      = $partialcount->rowCount();
	
	//Sırada / Sipariş Alındı
	$pendingcount      = $conn->prepare("SELECT * FROM orders WHERE order_status=:order_status && order_where=:where");
    $pendingcount     -> execute(array("order_status"=>"pending","where"=>"api"));
    $pendingcount      = $pendingcount->rowCount();
	
	//Gönderim Sırasında
	$processingcount      = $conn->prepare("SELECT * FROM orders WHERE order_status=:order_status && order_where=:where");
    $processingcount     -> execute(array("order_status"=>"processing","where"=>"api"));
    $processingcount      = $processingcount->rowCount();
	
	
	//İptal Edildi
	$canceledcount      = $conn->prepare("SELECT * FROM orders WHERE order_status=:order_status && order_where=:where");
    $canceledcount     -> execute(array("order_status"=>"cancelled","where"=>"api"));
        ...
        elseif( route(2) == "multi-action" ):
          ...
          elseif( $action ==  "resend" ):
            foreach ($orders as $id => $value):
              ...
              $update->execute(array("error"=>$error,"api"=>$order["id"],"serviceid"=>$order["api_service"],"orderid"=>$order_id,"detail"=>json_encode($get_order),"id"=>$order["order_id"],"profit"=>$order["order_charge"]-$api_charge,"api_charge"=>$api_charge,"api_currencycharge"=>$currencycharge ));
            endforeach;
          endif;
          header("Location:".site_url("admin/orders"));
        endif;
  ...
  if( route(2) == "order_cancel" ):
    ...
    header("Location:".site_url("admin/orders"));
  elseif( route(2) == "order_refill_activate" ):
    $id     = route(3);
    $update = $conn->prepare("UPDATE orders SET refill=:refill WHERE order_id=:id ");
    $update->execute(array("refill"=>"1","id"=>$id));
